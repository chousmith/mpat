...
var service = server.listen(port, function(request, response) {
	if(request.method == 'POST' && request.post.url) {
		// grab the URL from the POST var(s)
		var url = request.post.url;

		// check for plumbingDetails?!
		var plumbingDetails = false;
		BUFFER_TIME = NODETAIL_BUFFER_TIME;
		if ( request.post.plumbingDetails ) {
			if ( request.post.plumbingDetails == 1 ) {
				plumbingDetails = true;
				BUFFER_TIME = DETAILED_BUFFER_TIME;
			}
		}
		// check for emotionDetails?!
		var emotionDetails = false;
		if ( request.post.emotionDetails ) {
			if ( request.post.emotionDetails == 1 ) {
				emotionDetails = true;
			}
		}
